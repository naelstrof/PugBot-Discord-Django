...
NOAUTHMESSAGE = 'Failed to complete request: This bot has incorrect authorization. Please install a working token. (See README.md for more details).'
...
def verify(request):
    if not request.user.is_authenticated():
        return HttpResponse(json.dumps({'message': NOAUTHMESSAGE}))
...
def list_all(request):
    if not request.user.is_authenticated():
        return HttpResponse(json.dumps({'message_general': NOAUTHMESSAGE}))
    incoming_message = json.loads(request.body.decode('utf-8'))
    pprint(incoming_message)
...
def listing(request):
    if not request.user.is_authenticated():
        return HttpResponse(json.dumps({'message_specific': NOAUTHMESSAGE}))
    incoming_message = json.loads(request.body.decode('utf-8'))
    pprint(incoming_message)
    for key,value  in incoming_message.items():
        if key:
