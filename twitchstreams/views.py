...
from twitchstreams.twitchapi import TwitchAPI
...
STREAM = 'stream'
CHAT = 'chat'
CLIENT_ID = 'hbf4f6yuzu9osy2rc8jfhuv8ibwr1c'
...
def view_streams(request, streams_url=''):
	...
	c = RequestContext(request, {
		...
		'edit_url': 'https://ut4pugs.us/multi/' + 'edit/%s/' % ('/'.join(streams).lower()),
		'client_id': CLIENT_ID,
	})
...
def view_tag(request, tag = ''):
	...
	if len(tags) == 1 and tags[0].current():
		...
		if len(channels):
			...
			return HttpResponseRedirect('/' +  '#featured/' + tag.replace(' ', '-') + '/')
	else:
